            ...
            <div style={{ background: 'var(--ex-carbon-900)', border: '1px solid var(--ex-carbon-700)', borderRadius: 8, padding: 14 }}>
              ...
              <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
                {[0, 0.25, 0.5, 0.75, 1].map((g) => {
                  ...
                  return (<g key={g}>
                    <line x1={PAD} y1={y} x2={W - PAD} y2={y} stroke="var(--ex-carbon-700)" strokeWidth="1" />
                    <text x={4} y={y + 3} fill="var(--ex-graphite-600)" fontSize="9" fontFamily="var(--font-mono)">{Math.round(1600 * (1 - g))}</text>
                  </g>);
                })}
                <path d={`${altPath} L ${W - PAD} ${H - PAD} L ${PAD} ${H - PAD} Z`} fill="color-mix(in srgb, var(--ex-aqua-500) 8%, transparent)" stroke="none" />
                <path d={altPath} fill="none" stroke="var(--ex-aqua-500)" strokeWidth="2" />
                <line x1={dotX} y1={PAD} x2={dotX} y2={H - PAD} stroke="var(--ex-aqua-500)" strokeWidth="1" strokeDasharray="3 4" opacity=".5" />
                <circle cx={dotX} cy={dotY} r="5" fill="var(--ex-hud-green)" stroke="var(--ex-carbon-950)" strokeWidth="1.5" />
              </svg>
            </div>
            {/* telemetry chart */}
            <div style={{ background: 'var(--ex-carbon-900)', border: '1px solid var(--ex-carbon-700)', borderRadius: 8, padding: 14 }}>
              ...
              <svg viewBox={`0 0 ${CW} ${CH}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
                ...
                <line x1={scrubX} y1={4} x2={scrubX} y2={CH - 4} stroke="var(--ex-hud-green)" strokeWidth="1" />
              </svg>
            </div>
          </div>
