      ...
      <div style={{ border: '2px solid ' + (armed ? 'var(--ex-field-500)' : 'var(--ex-amber-500)'), borderRadius: 8, background: 'var(--surface-card)', padding: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {armed
            ...
            : <Button variant="primary" iconLeft={<Icon name="shield" size={18} />} disabled title="Resolve the three open readiness items before arming">Arm system</Button>}
          <Badge tone={armed ? 'ready' : 'caution'}>{armed ? 'Armed' : 'Blocked · 3 items'}</Badge>
        </div>
      </div>
    );
  }
