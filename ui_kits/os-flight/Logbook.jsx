...
(function () {
  ...
  function FlightDrawer({ mode, flight, onClose, onCreate, onImport, onOpenImport, onOpenSignal }) {
    ...
    if (saved) return <div className="flight-backdrop"><aside ref={dialogRef} className="flight-drawer" role="dialog" aria-modal="true" aria-label="Flight saved"><div className="flight-success"><div><span className="flight-success-icon"><Icon name="check" size={24}/></span><h2>{mode==='create'?'Flight ready for preparation':'Import started'}</h2><p>{mode==='create'?'The flight is now available for data import or a linked Signal session.':'The source files are attached and the flight has moved to Processing.'}</p><Button autoFocus onClick={onClose}>Done</Button></div></div></aside></div>;
    const commit = () => { if(mode==='create') onCreate({aircraft,zone,date}); else onImport({aircraft,zone,source}); setSaved(true); };
    return <div className="flight-backdrop" onMouseDown={(e)=>{if(e.target===e.currentTarget)onClose();}}><aside ref={dialogRef} className="flight-drawer" role="dialog" aria-modal="true" aria-label={title}>
  ...
  function Field({label,value,onChange,full}) { const id=React.useId(); return <div className={`flight-field ${full?'full':''}`}><label htmlFor={id}>{label}</label><input id={id} value={value} onChange={(e)=>onChange(e.target.value)}/></div>; }
  function SelectField({label,value,onChange,options}) { const id=React.useId(); return <div className="flight-field"><label htmlFor={id}>{label}</label><select id={id} value={value} onChange={(e)=>onChange(e.target.value)}>{options.map((o)=><option key={o}>{o}</option>)}</select></div>; }
