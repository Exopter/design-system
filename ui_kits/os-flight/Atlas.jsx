...
(function () {
  ...
  const CSS = `
    .atlas-root{padding:20px;max-width:1180px;margin:0 auto}.atlas-head{display:flex;align-items:flex-end;gap:16px;margin-bottom:16px}.atlas-layout{display:grid;grid-template-columns:320px minmax(0,1fr);gap:14px}.atlas-zone-button:focus-visible{outline:2px solid var(--focus-ring);outline-offset:-2px}.atlas-zone-button[aria-pressed=true]{box-shadow:inset 3px 0 0 var(--ex-aqua-500)}
    @media(max-width:760px){.atlas-root{padding:16px}.atlas-head{align-items:flex-start;flex-wrap:wrap}.atlas-layout{grid-template-columns:1fr}.atlas-zone-list{max-height:280px;overflow:auto}.atlas-map{height:320px!important}.atlas-details{grid-template-columns:1fr!important}.atlas-details>div:first-child{border-right:0!important;border-bottom:1px solid var(--border-rule)}}
  `;
  if (!document.getElementById('atlas-screen-css')) { const s=document.createElement('style');s.id='atlas-screen-css';s.textContent=CSS;document.head.appendChild(s); }
    ...
    return (
      <div className="atlas-root">
        ...
        <div className="atlas-layout">
          <aside className="atlas-zone-list" style={PANEL}>
            <div style={{ padding: 14, borderBottom: '1px solid var(--border-rule)' }}><span style={EYEBROW}>Available sites</span></div>
            {ZONES.map((z, i) => (
              <button key={z.name} type="button" className="atlas-zone-button" aria-pressed={i === selected} onClick={() => setSelected(i)} style={{ width: '100%', padding: 14, textAlign: 'left', border: 0, borderBottom: '1px solid var(--border-rule)', background: i === selected ? 'var(--surface-hover)' : 'transparent', cursor: 'pointer' }}>
          ...
          <section style={{ ...PANEL, overflow: 'hidden' }}>
            <div className="atlas-map" style={{ height: 430, position: 'relative', backgroundImage: 'url(../../assets/landing-zone-terrain-v1.png)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
            ...
            <div className="atlas-details" style={{ display: 'grid', gridTemplateColumns: '1.25fr 1fr', gap: 0, borderTop: '1px solid var(--border-rule)' }}>
