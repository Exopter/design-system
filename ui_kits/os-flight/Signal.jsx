...
(function () {
  ...
  function SignalStartDrawer({ preparations, onClose, onStart }) {
    ...
    return <Drawer title="Start live session" eyebrow="Signal acquisition" description="Choose how this session should be attached to a Flight." onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>Cancel</Button><Button iconLeft={<Icon name="radio" size={16}/>} onClick={()=>onStart({mode,flightId,aircraft:'EXO-001'})}>Start acquisition</Button></>}><div className="signal-choice"><button type="button" data-active={mode==='existing'} aria-pressed={mode==='existing'} onClick={()=>setMode('existing')}><Icon name="clipboard-check" size={19}/><strong>Use prepared Flight</strong><span>Attach Signal to a Flight already in Preparation.</span></button><button type="button" data-active={mode==='automatic'} aria-pressed={mode==='automatic'} onClick={()=>setMode('automatic')}><Icon name="plus" size={19}/><strong>Create automatically</strong><span>Create a Flight as soon as acquisition starts.</span></button></div>{mode==='existing'?<><Select label="Prepared Flight" value={flightId} onChange={(e)=>setFlightId(e.target.value)} options={preparations.map((flight)=>({value:flight.id,label:`${flight.id} · ${flight.aircraft}`}))}/><div className="signal-help">The selected Flight moves from Preparation to Live and receives this Signal session automatically.</div></>:<><div className="signal-detect"><div><span style={{fontFamily:'var(--font-mono)',fontSize:10,fontWeight:600,letterSpacing:'.07em',textTransform:'uppercase',color:'var(--text-muted)'}}>Aircraft detected</span><strong>EXO-001</strong></div></div><div className="signal-help">Signal creates a Flight with default values. The detected aircraft is filled now; an unresolved aircraft is stored as To complete without blocking capture.</div></>}</Drawer>;
  }
