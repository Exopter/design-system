...
(function () {
  ...
  const CSS = `
    ...
    .signal-tool-button:focus-visible,.signal-widget-mode:focus-visible,.signal-drag-handle:focus-visible,.signal-close:focus-visible,.signal-choice button:focus-visible,.signal-lz:focus-visible{outline:2px solid var(--focus-ring);outline-offset:2px}
  ...
  function UnifiedDashboard({ panelModes, onPanelMode, flight, cloudLive, events, onNavigate }) {
    ...
    return <main ref={boardRef} className="signal-dashboard" aria-label="Movable live Signal widgets">
      <WorkspacePanel id="map" title="Live map" subtitle="Flight path, heading and landing zone" icon="map" mode={panelModes.map} style={panelStyle('map')} dragging={draggingPanel==='map'} onMode={changeMode} {...panelInteraction}><UnifiedMap key={`map-${panelModes.map}-${layoutKey}`} flight={flight} events={events} onNavigate={onNavigate}/></WorkspacePanel>
  ...
  function FlightPathCanvas({ progress }) {
    ...
    React.useEffect(() => {
      ...
      ctx.strokeStyle=hud;ctx.lineWidth=3;ctx.setLineDash([7,6]);ctx.beginPath();points.slice(2).forEach(([px,py],i)=>{const x=px*w,y=py*h;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});ctx.stroke();ctx.setLineDash([]);
