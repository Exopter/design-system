...
This prototype keeps three rooms in the primary operator rail, ordered Flights, Hangar, then a separated Signal entry. Each room owns a different operational question and a different source of truth.

| Room | Operator question | Owns | Does not own |
| --- | --- | --- | --- |
| Flights | What are we preparing, and what happened? | Flight lifecycle, preparation, optional location, logbook, sealed flight record, replay, analysis, notes | Serialized hardware, live radio acquisition |
...
- A flight preparation selects one aircraft configuration from Hangar and may record a free-text location.
- A manually created Flight starts in Preparation and can receive FlySight or ExoFDR data from removable media.
- A direct import creates a Flight after the operator confirms or completes its aircraft.
- Starting Signal from a prepared Flight attaches the live session to it automatically.
- Starting Signal without a selected prepared Flight creates one automatically. A detected aircraft is filled; an unresolved aircraft remains `To complete` without blocking capture.
- Signal monitors the active flight and its aircraft association.
...
- Waypoints, routes, targets, and mission corridors are outside the current product scope.
...
- Heading, airspeed, altitude, vertical speed, and glide live in one shared telemetry strip instead of being repeated inside Map and Instruments.
- Flight lifecycle is `Preparation → Live or SD import → Processing → Analysed or Review`.
- The Live map shows the aircraft, heading, and continuously extending flight path.
